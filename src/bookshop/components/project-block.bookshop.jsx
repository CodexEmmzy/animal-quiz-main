...
export default function ProjectBlock({
  ...
  return (
    <div className="project-card">
      ...
      <ul className="mb-2">
        {objectives.map((o, i) => (
          <li key={i}>Objective: {o}</li>
        ))}
      </ul>
      <ul className="mb-2">
        {elements.map((e, i) => (
          <li key={i}>Element: {e}</li>
        ))}
      </ul>
      <ul className="mb-2">
        {scenarios.map((s, i) => (
          <li key={i}>Scenario: {s}</li>
        ))}
      </ul>
      <ul className="mb-2">
        {extensions.map((ex, i) => (
          <li key={i}>Extension: {ex}</li>
        ))}
      </ul>
      {featured && (
        <span className="inline-block bg-steel text-white px-2 py-1 rounded text-xs">
          Featured
        </span>
      )}
    </div>
